...
namespace Moe.Tools
{
	public static partial class GameTools
    {
        public static class Scene
        {
            public static List<T> GetAllComponents<T>(UScene scene, bool allowAtEditTime = false)
            {
                ...
                {
                    Debug.LogError(GameTools.String.Enclose(scene.name) + " must be loaded to be able to get its components");
                }
    ...
    public static partial class GameToolsExtensionMethods
    {
        ...
        {
            return GameTools.Scene.GetAllComponents<T>(scene, allowAtEditTime);
        }
    }
}